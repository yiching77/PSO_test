...
void UDPBroadcast::openRead()
{
  ...
  int opt = 1;
  int error = setsockopt(_readFd, SOL_SOCKET, SO_BROADCAST, (const char*)&opt, sizeof(opt));

  if (error != -1)
  {
    error = setsockopt(_readFd, SOL_SOCKET, SO_REUSEPORT, (const char*)&opt, sizeof(opt));
  }
...
void UDPBroadcast::openWrite()
{
  ...
  // Set broadcast permission
  int opt = 1;
  int error = setsockopt(_writeFd, SOL_SOCKET, SO_BROADCAST, (const char*)&opt, sizeof(opt));
  if (error == -1)
  {
...
void UDPBroadcast::broadcastMessage(const unsigned char* data, size_t len)
{
  ...
  for (size_t i = 0; i < _broadcastAddr.size(); i++)
  {
    ...
    int error = sendto(_writeFd, (const char*)data, len, MSG_DONTWAIT, (struct sockaddr*)&addr, sizeof(addr));
